...
public class KhachHangRepository {
    ...
    public List<KhachHang> getAllKhacHangDangHoatDong() {
        ...
        try {
            org.hibernate.query.Query query = session.createQuery("SELECT p FROM KhachHang p a  where p.maKhachHang <> 'KH0'");
    ...
    public BigDecimal getTongTienByIdKhachHang(String id) {
        ...
        try {
            Query query = session.createQuery("SELECT SUM(a.thanhTien) FROM HoaDon a where a.khachHang.id = :id");
